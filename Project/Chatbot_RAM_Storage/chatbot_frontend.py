...
if 'thread_display_names' not in st.session_state:
    st.session_state['thread_display_names'] = {}  # Maps thread_id to display name (first message preview)
...
if user_input:
    ...
    with st.chat_message('assistant'):
        ...
        ai_message = st.write_stream(
            message_chunks.content for message_chunks, metadata in chatbot.stream({'messages': [HumanMessage(content=user_input)]}, config=CONFIG, stream_mode='messages')
        )
