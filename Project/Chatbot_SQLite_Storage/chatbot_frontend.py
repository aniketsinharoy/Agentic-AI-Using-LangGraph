...
if user_input:
    ...
    with st.chat_message('assistant'):
        ...
        for message_chunks, metadata in chatbot.stream({'messages': [HumanMessage(content=user_input)]}, config=CONFIG, stream_mode='messages'):
            if hasattr(message_chunks, 'content'):
                full_response += message_chunks.content
